...
class AnymalGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.commands, self.gravity_vec, self.actions)
...
def compute_reward(
    ...
    # Inputs must be attributes from the environment:
    # - self.root_states: [N, 13] (pos[3], quat[4], lin_vel[3], ang_vel[3])
    # - self.commands:    [N, 3]  (vx_cmd, vy_cmd, yaw_rate_cmd)
    # - self.gravity_vec: [N, 3]  (typically [0, 0, -1] per env)
    # - self.actions:     [N, A]
    device = root_states.device

    # Unscaled base velocities in the body frame
    base_quat = root_states[:, 3:7]
    world_lin_vel = root_states[:, 7:10]
    world_ang_vel = root_states[:, 10:13]
    base_lin_vel = quat_rotate_inverse(base_quat, world_lin_vel)  # [N,3]
    base_ang_vel = quat_rotate_inverse(base_quat, world_ang_vel)  # [N,3]

    # Velocity tracking errors (x, y in body frame, yaw rate around z-axis)
    vel_err_xy = base_lin_vel[:, 0:2] - commands[:, 0:2]  # [N,2]
    yaw_err = base_ang_vel[:, 2] - commands[:, 2]         # [N]

    # Uprightness via projected gravity in base frame (z ~ -1 when upright)
    projected_gravity = quat_rotate(base_quat, gravity_vec)  # [N,3]
    tilt_measure = 1.0 + projected_gravity[:, 2]  # 0 when perfectly upright

    # Exponential shaping temperatures (each component has its own temperature)
    temp_lin: float = 0.30       # m^2/s^2 scale for linear velocity error
    temp_yaw: float = 0.25       # (rad/s)^2 scale for yaw rate error
    temp_upright: float = 0.10   # scale for tilt measure squared
    temp_action: float = 4.0     # scale for action magnitude regularization

    # Reward components in [0, 1] via exponential shaping
    lin_err_sq = vel_err_xy[:, 0] * vel_err_xy[:, 0] + vel_err_xy[:, 1] * vel_err_xy[:, 1]
    r_lin = torch.exp(-lin_err_sq / temp_lin)

    r_yaw = torch.exp(-(yaw_err * yaw_err) / temp_yaw)

    r_upright = torch.exp(-(tilt_measure * tilt_measure) / temp_upright)

    act_mag = torch.sum(actions * actions, dim=1)
    r_action = torch.exp(-act_mag / temp_action)
    ...
    components: Dict[str, torch.Tensor] = {
        "lin_vel_tracking": r_lin,
        "yaw_rate_tracking": r_yaw,
        "upright": r_upright,
